...
export default (client, options = {}) => {
    ...
    function readFile(file) {
        const service = client.service('uploads');
        return new Promise((resolve, reject) => {
            const fr = new FileReader();
            fr.onload = () => {
                // resolve(fr.result)
                resolve(service.create({uri: fr.result}))
            };
            fr.onerror = reject;
            fr.readAsDataURL(file.rawFile);
        });
    }
        ...
        switch (type) {
            ...
            case UPDATE:
                if (usePatch) {
                    ...
                    if (data.upload) {
                        console.log(data)
                        readFile(data.upload).then((result) => {
                            data.upload = result._id;
                            console.log(data)
            ...
            case CREATE:
                if (params.data.upload) {
                    return new Promise((resolve, reject)=>{
                        readFile(params.data.upload).then((result) => {
                            delete params.data.upload.rawFile;
                            params.data.upload.src = result._id;
                            resolve(service.create(params.data));
                        })
                    })
                } else {
